var requestWebcam=function(c){this.defaults={video_in:null,video_out:null,name:["video_",(new Date()+"").slice(4,28),".mp4"].join(""),required_audio:true,required_video:true,onGetPermission:function(){},onForgetPermission:function(){},onDeniedPermission:function(d){console.log(d.name+": "+d.message);
...
};requestWebcam.prototype.getRecorder=function(){return this.recorder;};requestWebcam.prototype.getBlob=function(){return this.blob;};requestWebcam.prototype.getRequired_audio=function(){return this.required_audio;
};requestWebcam.prototype.getRequired_video=function(){return this.required_video;};requestWebcam.prototype.forgetPermission=function(){for(var b=0;b<this.stream.getTracks().length;
b++){this.stream.getTracks()[b].stop();}if(this.onForgetPermission&&typeof this.onForgetPermission==="function"){this.onForgetPermission();}};requestWebcam.prototype.requestPermission=function(){var b=this;
navigator.mediaDevices.getUserMedia({video:true,audio:true}).then(function(c){b.stream=c;if(b.stream.getVideoTracks().length==0&&b.required_video){b.onDeniedPermission({name:"Required Video",message:"No video device found."});
b.forgetPermission();}else{if(b.stream.getAudioTracks().length==0&&b.required_audio){b.onDeniedPermission({name:"Required Audio",message:"No audio device found."});
b.forgetPermission();}else{if(b.video_in){b.video_in.src=URL.createObjectURL(b.stream);}if(b.onGetPermission&&typeof b.onGetPermission==="function"){b.onGetPermission();
